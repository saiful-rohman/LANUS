package com.javaindo.lautnusantara.view.setting_user
...
import com.javaindo.lautnusantara.databinding.ActivitySettingUserBinding
...
import com.javaindo.lautnusantara.viewmodel.SettingUserViewModel
...
class SettingUserActivity : AppCompatActivity() {
    ...
    private fun setData(data : SettingUserModel){
        if(data != null){
            binding.txvBBM.text = data.bbmConsume
            binding.txvMileage.text = data.mileage
            binding.txvSpeed.text = data.speed
            binding.txvBrandEngine.text = data.brandEngine
            binding.txvEngine.text = data.engine
        }
    }

    private fun onClick(){

        val bbmList = listOf("LITER","GALON")
        binding.imgBBM.setOnClickListener {
            dialogChoice(0,bbmList)
        }

        val bbmMileage = listOf("NM","M","KM","MIL")
        binding.imgMileage.setOnClickListener {
            dialogChoice(1,bbmMileage)
        }

        val bbmSpeed = listOf("KNOT","KMH","MPH")
        binding.imgSpeed.setOnClickListener {
            dialogChoice(2,bbmSpeed)
        }
        ...
        binding.imgBrandEngine.setOnClickListener {
            dialogChoice(3,bbmBrandEngine)
        }
        ...
        binding.imgEngine.setOnClickListener {
            dialogChoice(4,bbmEngine)
        }
    ...
    private fun dialogChoice(indx : Int, chooseList : List<String>){
        var titleDialog = ""
        if(indx == 0){
            titleDialog = "Pilih BBM"
        } else if(indx == 1){
            titleDialog = "Pilih Satuan"
        } else if(indx == 2){
            titleDialog = "Pilih Satuan"
        } else if(indx == 3){
...
//        val lnrDialogContent = findViewById<LinearLayout>(R.id.lnrDialogContent)
        ...
        if(chooseList != null && chooseList.size > 0){
            for(i in chooseList.indices){
                ...
                lnrChoices.get(i).layoutParams = LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT)
                ...
                txvChoices.get(i).layoutParams = LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT)
                txvChoices.get(i).textSize = 14F
                txvChoices.get(i).setText(chooseList.get(i))
                var valChoosen = ""
                if(indx == 0){
                    valChoosen = binding.txvBBM.text.toString()
                } else if(indx == 1){
                    valChoosen = binding.txvMileage.text.toString()
                } else if(indx == 2){
                    valChoosen = binding.txvSpeed.text.toString()
                } else if(indx == 3){
                    valChoosen = binding.txvBrandEngine.text.toString()
                } else if(indx == 4){
                    valChoosen = binding.txvEngine.text.toString()
                }
//                var strChoice = chooseList.get(i)
                ...
                lnrChoices.get(i).setOnClickListener {
                    if(indx == 0){
                        binding.txvBBM.text = chooseList.get(i)
                    } else if(indx == 1){
                        binding.txvMileage.text = chooseList.get(i)
                    } else if(indx == 2){
                        binding.txvSpeed.text = chooseList.get(i)
                    } else if(indx == 3){
                        binding.txvBrandEngine.text = chooseList.get(i)
                    } else if(indx == 4){
                        binding.txvEngine.text = chooseList.get(i)
                    }

                    val data  = SettingUserModel(
                        1,
                        binding.txvBBM.text.toString(),
                        binding.txvMileage.text.toString(),
                        binding.txvSpeed.text.toString(),
                        binding.txvBrandEngine.text.toString(),
                        binding.txvEngine.text.toString()
                    )
                    viewModel.saveSetting(data)
